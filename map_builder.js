...
//lots of args... w/e
function createSidingHelper(v1, v2, v3, v4, ind, sign, counter, groundElevation, vertices, textureCoords, vertexIndices) {
	if (ind == 0 || ind == 1) {
	...
	for (var i = 0; i < tileNum; i++) {
		for (var j = 0; j < tileNum; j++) {
			if (i > 0 && -sign*(groundElevations[i*tileNum+j] - groundElevations[(i-1)*tileNum+j]) > 0) {
				...
				createSidingHelper(v1,v2,v3,v4, ind, sign, counter, groundElevations[i*tileNum + j],
						vertices, textureCoords, vertexIndices);
				counter += 1;
			}
